...
import random
import argparse
# python tracing_composition1.py --dataset composition1_2000_200_12.6
def main():
    
    parser = argparse.ArgumentParser()

    parser.add_argument("--dataset", default=r'composition1.2000.200.12.6', type=str, required=True, help="dataset name")
    ...
    parser.add_argument("--save_path", default=r'output/causal_result.json', type=str, help="path to save result")
    ...
    split = 'test_inferred_ood' #ood b appear later layer
    rand_inds = np.random.choice(len(d[split]), 300, replace=False).tolist()[:20]
    ...
    for checkpoint in tqdm(all_checkpoints[-1:]):
        ...
        for index in tqdm(rand_inds):
            ...
            b = atomic_dict[(h, n_r)]
            t = atomic_dict[(b, r)]
            # TODO:check the second hop
            decoder_temp = tokenizer([query+"<"+b+">"], return_tensors="pt", padding=True)
            ...
            # print(len(all_hidden_states),all_hidden_states[target_layer][0, :, :].shape)
            rank_before = return_rank(all_hidden_states[target_layer][0, :, :], word_embedding, tokenizer("<"+b+">")['input_ids'][0])[-2]
            ...
            # MRRs
            for layer_ind in range(1, target_layer):
                ...
                res_dict['b_rank_pos3_'+str(layer_ind)] = return_rank(temp[0, :, :], word_embedding, tokenizer("<"+b+">")['input_ids'][0])[3]
                res_dict['r2_rank_pos2_'+str(layer_ind)] = return_rank(temp[0, :, :], word_embedding, tokenizer("<"+r+">")['input_ids'][0])[2]
                res_dict['r2_rank_pos3_'+str(layer_ind)] = return_rank(temp[0, :, :], word_embedding, tokenizer("<"+r+">")['input_ids'][0])[3]
            ...
            for layer_to_intervene in range(0, target_layer):
                ...
                # print("--------")
                rank_after = return_rank(hidden_states[0, :, :], word_embedding, tokenizer("<"+b+">")['input_ids'][0])[-2]
                ...
                res_dict['t_h_'+str(layer_to_intervene)] = rank_after
            ...
            for layer_to_intervene in range(0, target_layer):
                ...
                rank_after = return_rank(hidden_states[0, :, :], word_embedding, tokenizer("<"+t+">")['input_ids'][0])[-1]
                res_dict['t_r1_'+str(layer_to_intervene)] = rank_after
            ...
            for layer_to_intervene in range(0, target_layer):
                ...
                rank_after = return_rank(hidden_states[0, :, :], word_embedding, tokenizer("<"+t+">")['input_ids'][0])[-1]
                res_dict['t_r2_'+str(layer_to_intervene)] = rank_after

            # perturb the first-output token
            # wait for ...
            all_ = set()
            for head in h2rt_train:
                if (head, n_r) not in atomic_dict:
                ...
                if tail != b:
                    if (tail, r) not in atomic_dict or atomic_dict[(tail, r)] != t:
                        all_.add(head)
            query = "<{}><{}><{}><{}>".format(h, n_r, b, random.choice(list(all_)))
            ...
            for layer_to_intervene in range(0, target_layer):
                ...
                # print("--------")
                rank_after = return_rank(hidden_states[0, :, :], word_embedding, tokenizer("<"+b+">")['input_ids'][0])[-2]
                res_dict['b_b_'+str(layer_to_intervene)] = rank_after
                rank_after = return_rank(hidden_states[0, :, :], word_embedding, tokenizer("<"+t+">")['input_ids'][0])[-1]
                res_dict['t_b_'+str(layer_to_intervene)] = rank_after
