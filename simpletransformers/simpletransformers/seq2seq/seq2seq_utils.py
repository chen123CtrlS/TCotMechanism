...
def preprocess_data_bart(data):
    ...
    if len(target_prefix_ids)>2:
        lm_labels[:2] = -100  # TODO: no loss on prefix tokens but <mask> position
        lm_labels[2:len(target_prefix_ids)] = -100
        # print(lm_labels,target_ids)
    else:
        lm_labels[:len(target_prefix_ids)] = -100
    return {
